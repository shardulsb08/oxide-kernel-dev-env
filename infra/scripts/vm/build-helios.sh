...
Building Helios/illumos in guest '$vm' ($ip) as $user.
  steps:   clone (if needed) -> gmake setup OXIDE_STAFF=no -> gmake illumos
  output:  /data/helios/helios  (in the guest; on the persistent data disk)
  log:     $log  (host copy; streamed below)

This is long-running. It streams live; if your SSH drops, re-run -- the
steps are idempotent (setup is skipped once ./helios-build exists). For a
detach-proof run, start it inside tmux/screen in the guest instead.

EOF

# Pass FORCE_SETUP through if set. No pty (-t) so non-interactive pkg/rustup
# steps don't expect one; output streams and is teed to the host log.
ssh -o StrictHostKeyChecking=accept-new "$user@$ip" \
    "FORCE_SETUP='${FORCE_SETUP:-0}' sh -s" < "$SCRIPT_DIR/guest/build-helios.sh" 2>&1 \
    | tee "$log"
